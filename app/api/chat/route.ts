...
export async function POST(request: Request) {
  try {
    ...
    const systemPrompt = productContext
      ? `${SYSTEM_PROMPT}\n\nKhách hàng đang xem sản phẩm:\n- Tên: ${productContext.name}\n- Giá: ${productContext.price}\n- Danh mục: ${productContext.category}${productContext.description ? `\n- Mô tả: ${productContext.description}` : ''}\n\nHãy ưu tiên tư vấn về sản phẩm này.`
      : SYSTEM_PROMPT;
