...
export default function BusinessStorefront() {
  ...
  const { getBySeller, getSellerProfile } = useProducts();
  const [activeCategory, setActiveCategory] = useState("tat-ca");

  const profile = getSellerProfile(sellerId);
  const allProducts = getBySeller(sellerId).filter((p) => p.status === "approved");
  ...
  const filtered = useMemo(() => {
    if (activeCategory === "tat-ca") return allProducts;
    return allProducts.filter((p) => p.category === activeCategory);
  }, [allProducts, activeCategory]);

  if (!profile && allProducts.length === 0) {
    return (
  ...
  const displayName = profile?.name ?? allProducts[0]?.sellerName ?? "Doanh nghiệp";
              ...
              <div className="flex items-center gap-6 mt-4 flex-wrap">
                ...
                <div className="text-center">
                  <div className="text-xl font-bold">
                    {allProducts.reduce((s, p) => s + p.sold, 0).toLocaleString()}
                  </div>
