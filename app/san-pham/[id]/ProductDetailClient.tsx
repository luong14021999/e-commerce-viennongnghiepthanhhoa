      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3">
          <nav className="text-xs text-gray-500 flex items-center gap-1 flex-wrap">
            ...
            <span>›</span>
            <Link href="/san-pham" className="hover:text-green-700">{isService ? "Dịch vụ" : "Sản phẩm"}</Link>
            <span>›</span>
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-10">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              {isService ? (
                <span className="text-sm text-gray-500">| {p.sold.toLocaleString()} lượt sử dụng</span>
              ) : (
            ...
            {isService ? (
              ...
              <>
                <div className="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-5">
                  <p className="text-blue-800 font-semibold text-lg mb-1">Liên hệ để biết giá dịch vụ</p>
                  <p className="text-blue-600 text-sm mb-4">Báo giá theo quy mô và yêu cầu cụ thể của đơn vị.</p>
                  <a
                    href="tel:0929606568"
                ...
                <div className="mb-5 space-y-2 text-sm">
                  <div className="flex items-center gap-2">
                    <span className="text-gray-500 w-28 flex-shrink-0">Đơn vị cung cấp:</span>
                    <span className="font-medium text-gray-800">{p.origin}</span>
                  </div>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-gray-500 w-28 flex-shrink-0">Chứng nhận:</span>
                    <div className="flex flex-wrap gap-1.5">
                      {p.certifications.map((c) => (
                        <span key={c} className="bg-blue-100 text-blue-800 text-xs font-medium px-2 py-0.5 rounded-full border border-blue-200">
                          ✓ {c}
                        </span>
                      ))}
                    </div>
            ...
            <ul className="space-y-2">
              {p.specs.map((s) => (
                <li key={s} className="flex items-center gap-2 text-sm text-gray-700">
                  <svg className={`w-4 h-4 flex-shrink-0 ${isService ? "text-blue-600" : "text-green-600"}`} fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="space-y-4">
            {isService ? (
              <div className="bg-blue-50 rounded-2xl border border-blue-200 p-5">
                <h3 className="text-base font-bold text-blue-900 mb-3">Liên hệ tư vấn</h3>
                <ul className="space-y-3 text-sm mb-4">
                  {[
                    { icon: "📞", text: "Hotline: 0929 606 568" },
                    { icon: "✉️", text: "info@viennongnghiep.vn" },
                    { icon: "📍", text: "Số 1 Lê Hoàn, TP. Thanh Hóa" },
                    { icon: "🕐", text: "Hỗ trợ: 7:30–17:00 (T2–T7)" },
                  ].map((item) => (
                    <li key={item.text} className="flex items-center gap-2 text-blue-800">
                      <span>{item.icon}</span>
                      {item.text}
                    </li>
                  ))}
                </ul>
                <a href="tel:0929606568" className="block text-center bg-blue-700 text-white font-bold text-sm py-2.5 rounded-xl hover:bg-blue-600 transition-colors">
                  📞 Gọi ngay tư vấn
                </a>
              </div>
            ) : (
            ...
            <div className="bg-green-50 rounded-2xl border border-green-200 p-5">
              ...
              <a href="tel:0929606568" className="block text-center bg-green-700 text-white font-bold text-sm py-2.5 rounded-xl hover:bg-green-600 transition-colors">
                📞 0929 606 568
              </a>
            </div>
          </div>
