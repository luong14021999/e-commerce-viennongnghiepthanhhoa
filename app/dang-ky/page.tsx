...
export default function RegisterPage() {
  ...
  async function handleSubmit(e: { preventDefault(): void }) {
    ...
    const result = role === "buyer"
      ...
      : await registerBusiness({ name: form.name, phone: form.phone, email: form.email, password: form.password, businessName: form.businessName, taxCode: form.taxCode, businessAddress: form.businessAddress, category: form.category, description: form.description });
    setLoading(false);
    if (!result.ok) { setError(result.error ?? "Đăng ký thất bại"); return; }
      ...
      <div className="w-full max-w-lg">
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center gap-3">
            <Image src="/logo.png" alt="Viện Nông Nghiệp Thanh Hóa" width={88} height={88} className="flex-shrink-0 object-contain" />
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <button
            ...
            className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 transition-all ${
              role === "buyer" ? "border-green-600 bg-green-50" : "border-gray-200 bg-white hover:border-green-300"
            }`}
          >
            <span className="text-4xl">🛒</span>
            <span className="font-bold text-gray-800">Người mua</span>
            <span className="text-xs text-gray-500 text-center">Mua sắm sản phẩm nông nghiệp</span>
            {role === "buyer" && <span className="text-green-600 text-xs font-semibold">✓ Đã chọn</span>}
          </button>
          <button
            type="button"
            onClick={() => setRole("business")}
            className={`flex flex-col items-center gap-2 p-5 rounded-2xl border-2 transition-all ${
              role === "business" ? "border-green-600 bg-green-50" : "border-gray-200 bg-white hover:border-green-300"
            }`}
          >
            <span className="text-4xl">🏪</span>
            <span className="font-bold text-gray-800">Doanh nghiệp</span>
            <span className="text-xs text-gray-500 text-center">Đăng bán sản phẩm của bạn</span>
            {role === "business" && <span className="text-green-600 text-xs font-semibold">✓ Đã chọn</span>}
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1.5">Email <span className="text-gray-400 font-normal">(tùy chọn)</span></label>
            ...
            {role === "business" && (
              <>
                ...
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-1.5">Tên doanh nghiệp / HTX / Hộ kinh doanh <span className="text-red-500">*</span></label>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Mã số thuế / ĐKKD <span className="text-red-500">*</span></label>
                    <input type="text" value={form.taxCode} onChange={(e) => setField("taxCode", e.target.value)}
                      placeholder="2801234567"
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-green-500"/>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-gray-700 mb-1.5">Loại sản phẩm chính <span className="text-red-500">*</span></label>
                ...
                <div>
                  ...
                  <textarea value={form.description} onChange={(e) => setField("description", e.target.value)}
                    rows={3} placeholder="Mô tả ngắn về doanh nghiệp và sản phẩm bạn muốn bán..."
            ...
            <button type="submit" disabled={loading}
              ...
              {loading ? (
                <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>Đang đăng ký...</>
